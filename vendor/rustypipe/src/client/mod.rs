...
/// Optional host hook observing response headers before the body is consumed.
/// The hook must not retain or log account request/response data.
pub type ResponseObserver = Arc<dyn Fn(&Request, &Response) -> Result<(), Error> + Send + Sync>;

const MAX_RESPONSE_BYTES: usize = 16 * 1024 * 1024;

/// Bound decoded bytes while reading, including chunked/compressed responses
/// whose Content-Length is absent or describes only the encoded body.
async fn response_text(mut response: Response) -> Result<String, Error> {
    let mut body = Vec::new();
    while let Some(chunk) = response.chunk().await? {
        if chunk.len() > MAX_RESPONSE_BYTES.saturating_sub(body.len()) {
            return Err(Error::Other("response body exceeds size limit".into()));
        }
...
impl RustyPipeBuilder {
    ...
    /// Observe headers for host-managed browser session maintenance.
    #[must_use]
    pub fn response_observer(mut self, observer: ResponseObserver) -> Self {
...
impl RustyPipe {
    ...
    async fn http_request(&self, request: &Request) -> Result<Response, Error> {
        ...
        for n in 0..=self.inner.n_http_retries {
            ...
            let err = match resp {
                Ok(resp) => {
                    self.observe_response(request, &resp)?;
        ...
        last_resp.unwrap().map_err(Into::into)
    }

    fn observe_response(&self, request: &Request, response: &Response) -> Result<(), Error> {
        if let Some(observer) = &self.inner.response_observer {
    ...
    async fn http_request_estatus(&self, request: &Request) -> Result<Response, Error> {
        ...
        if status.is_client_error() || status.is_server_error() {
            let error_msg = if let Ok(body) = response_text(res).await {
    ...
    async fn http_request_txt(&self, request: &Request) -> Result<String, Error> {
        response_text(self.http_request_estatus(request).await?).await
    }
                ...
                if deobf_data.should_retry() {
                    ...
                    match DeobfData::extract(&self.inner.http, self.inner.reporter.as_deref()).await
                    {
                        ...
                        Err(e) => {
                            ...
                            if crate::deobfuscate::host_controlled() {
                                return Err(e);
                            }
    ...
    /// Install a host-validated browser session without network verification or
    /// persistence. Existing queries share this client, so hosts should create a
    /// new client when changing accounts or replacing cookies.
    pub fn set_web_session(
        ...
        if cookie.is_empty()
            || cookie.len() > 65_536
            || wreq::header::HeaderValue::from_str(&cookie).is_err()
            || session_index > 99
            || channel_syncid.as_ref().is_some_and(|id| {
                id.is_empty()
                    || id.len() > 256
                    || !id
                        .bytes()
        ...
        {
            return Err(Error::Other("invalid host browser session".into()));
        }
        let mut current = self
            ...
            .auth_cookie
            .write()
            .map_err(|_| Error::Other("browser session lock unavailable".into()))?;
        *current = Some(AuthCookie {
            cookie,
    ...
    /// Reuse the Music client version already obtained by the embedding host.
    /// This updates only the in-memory WEB_REMIX version cache.
    pub async fn set_music_client_version(&self, version: String) -> Result<(), Error> {
        if version.is_empty()
            || version.len() > 128
            || !version
                .bytes()
        ...
        {
            return Err(Error::Other("invalid Music client version".into()));
        }
        let cache = self
            .inner
            .cache
            .clients
            .get(&ClientType::DesktopMusic)
            .ok_or_else(|| Error::Other("Music client cache unavailable".into()))?;
        *cache.write().await = CacheEntry::from(ClientData { version });
        Ok(())
    }
...
impl RustyPipeQuery {
    ...
    async fn execute_request_attempt<R: MapJsonResponse<M> + Debug, M, B: Serialize + ?Sized>(
        ...
        self.client.observe_response(&request, &response)?;
...
mod host_session_tests {
    ...
    fn chunked_server(
        ...
        let server = std::thread::spawn(move || {
            ...
            if finish_body {
                ...
                // The reader must reject the oversized body without waiting
                // for the server to finish the transfer or close its socket.
        ...
        (request, release, server)
    }

    #[tokio::test(flavor = "current_thread")]
    async fn bounded_text_preserves_utf8_and_accepts_the_exact_limit() {
        ...
        for body in [
            ...
            let (request, release, server) = chunked_server(200, body, true);
            let response = client.execute(request).await.unwrap();
            let result = response_text(response).await;
            let _ = release.send(());
            server.join().unwrap();
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn bounded_text_stops_chunked_transfer_before_eof() {
        let client = Client::builder()
            .no_proxy()
            .timeout(Duration::from_secs(2))
            .build()
            .unwrap();
        let (request, release, server) =
            chunked_server(200, vec![b'x'; MAX_RESPONSE_BYTES + 1], false);
        let response = client.execute(request).await.unwrap();
        let result = response_text(response).await;
        let _ = release.send(());
        server.join().unwrap();
        assert!(
            matches!(result, Err(Error::Other(message)) if message == "response body exceeds size limit")
        );
    }

    #[tokio::test(flavor = "current_thread")]
    async fn oversized_error_response_preserves_http_status_without_its_body() {
        let client = RustyPipe::builder()
            ...
            .n_http_retries(0)
            .timeout(Duration::from_secs(2))
            .build_with_client(Client::builder().no_proxy())
            .unwrap();
        let (request, release, server) =
            chunked_server(403, vec![b'x'; MAX_RESPONSE_BYTES + 1], false);
        let result = client.http_request_txt(&request).await;
        let _ = release.send(());
        server.join().unwrap();
        assert!(matches!(result, Err(Error::HttpStatus(403, message)) if message.is_empty()));
    }

    #[tokio::test]
    async fn host_identity_is_used_in_music_request_headers_without_network() {
        let client = RustyPipe::builder()
            .no_storage()
            .no_reporter()
            .no_botguard()
            ...
            .unwrap();
        client
            .set_music_client_version("1.20260917.00.00".into())
            .await
            .unwrap();
        client
            .set_web_session(
                "SAPISID=synthetic-host-session".into(),
                3,
        ...
        let request = client
            .query()
            .authenticated()
            .request_builder(ClientType::DesktopMusic, "browse", None)
            .await
            .unwrap()
            .build()
            .unwrap();
        assert_eq!(request.url().host_str(), Some("music.youtube.com"));
        assert_eq!(request.headers().get("x-goog-authuser").unwrap(), "3");
        assert_eq!(
            request.headers().get("x-goog-pageid").unwrap(),
            "test-channel"
        );
        assert_eq!(
            request.headers().get("cookie").unwrap(),
            "SAPISID=synthetic-host-session"
        );
        ...
        assert!(!client.query().auth_enabled(ClientType::Tv));
        assert!(client.inner.storage.is_none());
        assert!(client.inner.reporter.is_none());
        assert!(client.inner.botguard.is_none());
        ...
        assert!(!anonymous.headers().contains_key("x-goog-pageid"));
        assert_eq!(anonymous.headers().get("cookie").unwrap(), CONSENT_COOKIE);
    }

    #[test]
    fn host_session_rejects_header_injection_and_invalid_identity() {
        let client = RustyPipe::builder()
            .no_storage()
            .no_reporter()
            .no_botguard()
            ...
            .unwrap();
        assert!(client
            .set_web_session("SAPISID=x\r\nX-Other: value".into(), 0, None)
            .is_err());
        assert!(client
            .set_web_session("SAPISID=x".into(), 100, None)
            .is_err());
        assert!(client
            .set_web_session("SAPISID=x".into(), 0, Some("invalid/channel".into()))
            .is_err());
        assert!(!client.query().auth_enabled(ClientType::DesktopMusic));
    }
}
