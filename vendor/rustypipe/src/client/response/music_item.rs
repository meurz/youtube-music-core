...
pub(crate) struct MusicCardShelf {
    #[serde(skip)]
    pub metadata: model::MusicMetadata,
    #[serde_as(as = "Text")]
    pub title: String,
...
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
// Keep the upstream queue representation without adding one allocation per track.
...
impl MusicListMapper {
    ...
    pub(crate) fn current_items(&self) -> &[MusicItem] {
        &self.items
    }
...
impl<'de> serde::Deserialize<'de> for ListMusicItem {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        let mut value = serde_json::Value::deserialize(deserializer)?;
        let metadata = super::music_metadata::extract(&value);
        super::music_metadata::normalize_unavailable_entry(&mut value);
        let mut item = Self::deserialize(value).map_err(serde::de::Error::custom)?;
        item.metadata = metadata;
        Ok(item)
    }
}

impl<'de> serde::Deserialize<'de> for CoverMusicItem {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
...
impl<'de> serde::Deserialize<'de> for MusicCardShelf {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        let value = serde_json::Value::deserialize(deserializer)?;
        let metadata = super::music_metadata::extract(&value);
        let mut item = Self::deserialize(value).map_err(serde::de::Error::custom)?;
...
impl<'de> serde::Deserialize<'de> for QueueMusicItem {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        let value = serde_json::Value::deserialize(deserializer)?;
        let metadata = super::music_metadata::extract(&value);
        let mut item = Self::deserialize(value).map_err(serde::de::Error::custom)?;
...
#[cfg(test)]
mod metadata_extension_tests {
    use super::*;
    use serde_json::json;

    #[test]
    fn unavailable_duplicate_entry_survives_typed_mapping() {
        let item: MusicResponseItem = serde_json::from_value(json!({
            "musicResponsiveListItemRenderer": {
                ...
                "menu":{"menuRenderer":{"items":[{"menuServiceItemRenderer":{"serviceEndpoint":{
                    "playlistEditEndpoint":{"playlistId":"PLfixture","actions":[{
                        "action":"ACTION_REMOVE_VIDEO","removedVideoId":"4D7u5KF7SP8","setVideoId":"entry-2"
                    }]}
                }}}]}}
            }
        })).unwrap();
        let mut mapper = MusicListMapper::new(Language::En);
        mapper.add_response_item(item);
        let mapped: MapResult<Vec<TrackItem>> = mapper.conv_items();
        assert_eq!(mapped.c.len(), 1);
        assert_eq!(mapped.c[0].id, "4D7u5KF7SP8");
        assert_eq!(mapped.c[0].metadata.available, Some(false));
        assert_eq!(
            mapped.c[0].metadata.playlist_id.as_deref(),
            Some("PLfixture")
        );
        assert_eq!(
            mapped.c[0].metadata.set_video_id.as_deref(),
            Some("entry-2")
        );
    }

    #[test]
    fn queue_metadata_is_deserialized_with_regular_typed_fields() {
        let item: QueueMusicItem = serde_json::from_value(json!({
            "videoId":"4D7u5KF7SP8", "title":{"simpleText":"Track"},
            "isPlayable":false, "setVideoId":"queue-entry",
            "badges":[{"icon":{"iconType":"MUSIC_EXPLICIT_BADGE"}}],
            "menu":{"menuRenderer":{"items":[],"likeStatus":"LIKE"}}
        }))
        .unwrap();
        let mapped = map_queue_item(item, Language::En).c;
        assert_eq!(mapped.metadata.available, Some(false));
        assert!(mapped.metadata.explicit);
        assert_eq!(mapped.metadata.rating.as_deref(), Some("LIKE"));
        assert_eq!(mapped.metadata.set_video_id.as_deref(), Some("queue-entry"));
    }
}
