...
pub struct TrackItem {
    /// Server-advertised Music action and playlist-entry metadata.
    #[serde(default)]
...
pub struct MusicArtist {
    /// Original shelf grouping, pagination and browse entry points.
    #[serde(default)]
    pub sections: Vec<MusicArtistSection>,
    /// Server-advertised Music action and playlist-entry metadata.
    #[serde(default)]
    pub metadata: MusicMetadata,
...
pub struct MusicSearchResult<T> {
    /// Original shelves with independent search continuations.
    #[serde(default)]
...
pub struct MusicSearchSuggestion {
    /// Whether the corresponding search term comes from account history.
    #[serde(default)]
...
/// Additional Web Music metadata required by native library editing clients.
#[derive(Debug, Default, Clone, Serialize, Deserialize, PartialEq, Eq)]
#[non_exhaustive]
pub struct MusicMetadata {
    /// Unique playlist entry ID; unlike the video ID this distinguishes duplicates.
    pub set_video_id: Option<String>,
    /// Playlist ID advertised by the item's menu or navigation endpoint.
    pub playlist_id: Option<String>,
    /// Whether an explicit-content badge is present.
    pub explicit: bool,
    /// Explicit availability state, or unknown when absent.
    pub available: Option<bool>,
    /// Server like status: LIKE, DISLIKE or INDIFFERENT.
    pub rating: Option<String>,
    /// Whether this item belongs to the current user's library.
    pub in_library: Option<bool>,
    /// Whether the current user subscribes to this artist.
    pub subscribed: Option<bool>,
    /// Whether the current user can edit this playlist.
    pub can_edit: Option<bool>,
    /// Opaque feedback token to add this item to the library.
    pub add_library_token: Option<String>,
    /// Opaque feedback token to remove this item from the library.
    pub remove_library_token: Option<String>,
}

/// A server-provided shelf on a Music artist page.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
#[non_exhaustive]
pub struct MusicArtistSection {
    /// Localized shelf title.
    pub title: String,
    /// Shelf items and an actual continuation token, if supplied.
    pub items: Paginator<MusicItem>,
    /// Separate browse destination advertised by a More button.
    pub browse_id: Option<String>,
    /// Parameters for that browse destination; never a continuation token.
    pub browse_params: Option<String>,
}

/// A server-provided shelf in a Music search result.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
#[non_exhaustive]
pub struct MusicSearchSection {
    /// Localized shelf title.
    pub title: String,
    /// Shelf contents and its independent search continuation.
    pub items: Paginator<MusicItem>,
}
