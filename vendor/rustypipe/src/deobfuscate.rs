...
type Interrupt = Arc<dyn Fn() -> bool + Send + Sync>;
thread_local! { static INTERRUPT: RefCell<Option<Interrupt>> = const { RefCell::new(None) }; }

pub(crate) fn host_controlled() -> bool {
    INTERRUPT.with(|slot| slot.borrow().is_some())
}

pub(crate) fn interrupted() -> bool {
    INTERRUPT.with(|slot| slot.borrow().as_ref().is_some_and(|check| check()))
}

pub(crate) fn with_interrupt<T>(interrupt: Interrupt, action: impl FnOnce() -> T) -> T {
    ...
    let _restore = Restore(INTERRUPT.with(|slot| slot.replace(Some(interrupt))));
    action()
}

fn bounded_runtime() -> Result<Runtime, rquickjs::Error> {
    let runtime = Runtime::new()?;
    runtime.set_memory_limit(256 * 1024 * 1024);
    runtime.set_max_stack_size(2 * 1024 * 1024);
    let deadline = Instant::now() + Duration::from_secs(30);
    let interrupt = INTERRUPT.with(|slot| slot.borrow().clone());
    runtime.set_interrupt_handler(Some(Box::new(move || {
        Instant::now() >= deadline || interrupt.as_ref().is_some_and(|check| check())
    })));
    Ok(runtime)
}
...
impl DeobfData {
    ...
    pub fn extract_fns(js_url: &str, player_js: &str) -> Result<Self, Error> {
        if player_js.len() > 8 * 1024 * 1024 || interrupted() {
            return Err(DeobfError::Other(
                "player extraction interrupted or input too large".into(),
            )
            .into());
        }
        ...
        let (sig_fn, nsig_fn) = match (get_sig_fn(player_js), get_nsig_fn(player_js)) {
            ...
            (Err(legacy_err), _) | (_, Err(legacy_err)) => {
                ...
                if interrupted() {
                    return Err(DeobfError::Other("player extraction interrupted".into()).into());
                }
...
impl Deobfuscator {
    ...
    pub fn deobfuscate_nsig(&self, nsig: &str) -> Result<String, DeobfError> {
        ...
        if res.is_empty() || res.starts_with("enhanced_except_") || res.ends_with(nsig) {
...
async fn get_response(http: &Client, url: &str) -> Result<String, Error> {
    ...
    while let Some(chunk) = resp.chunk().await? {
        if chunk.len() > (8 * 1024 * 1024usize).saturating_sub(body.len()) {
            return Err(Error::Other("player response exceeds size limit".into()));
        }
        body.extend_from_slice(&chunk);
    }
    String::from_utf8(body).map_err(|_| Error::Other("player response is not UTF-8".into()))
}
...
mod bounded_execution_tests {
    ...
    #[test]
    fn host_cancellation_interrupts_javascript_and_restores_scope() {
        let start = Instant::now();
        let check: Interrupt = Arc::new(move || start.elapsed() >= Duration::from_millis(10));
        let result = with_interrupt(check, || {
            let runtime = bounded_runtime().unwrap();
            let context = Context::full(&runtime).unwrap();
            context.with(|ctx| ctx.eval::<(), _>("while (true) {}"))
        });
    ...
    #[test]
    fn rejected_player_input_never_enters_the_solver() {
        let result = with_interrupt(Arc::new(|| true), || {
            DeobfData::extract_fns("https://www.youtube.com/s/player/public/base.js", "")
        });
        assert!(result.is_err());
        assert!(!interrupted());
    }
