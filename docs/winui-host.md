...
Version 0.9 provides the core protocol, session, discovery, library editing and
...
`ytmusic_abi_version` let the host check protocol 2.0 / ABI 2 locally.
...
Native SABR audio and optional official-browser attestation are available through
the [Web delivery interfaces](web-delivery.md). The optional
[Windows attestation helper](../examples/dotnet/web-attestation/README.md) generates
PO tokens for native playback. Native SABR returns media segments and needs a
compatible host demuxer for progressive playback. Audio playback uses the native
host; there is no browser playback bridge or CDM integration.
